...
#define LANE_ID_SUBSCRIPT_3 3
    ...
    static VO_PUB_ATTR_S VoPubAttrS = {0};
     ...
     * 关闭MIPI Tx设备
     */
    HI_VOID SampleCloseMipiTxFd(HI_S32 fd, SAMPLE_VO_CONFIG_S *VoConfig)
    {
        close(fd);
    ...
    HI_S32 SampleCommVoGetWhMipi(VO_INTF_SYNC_E enIntfSync, HI_U32 *pu32W, HI_U32 *pu32H, HI_U32 *pu32Frm)
    {
        ...
        case VO_OUTPUT_USER:
            ...
            *pu32Frm = 30; // 60: VO_OUTPUT_USER-Frame rate
            break;
        default:
    ...
    HI_S32 SampleCommVoStartDevMipi(VO_DEV VoDev, VO_PUB_ATTR_S *pstPubAttr)
    {
        ...
        HI_U32 u32Framerate = 30; // 60: device frame rate
    ...
    static HI_VOID StVoPubAttrCfg(SAMPLE_VO_CONFIG_S *pstVoConfig, VO_PUB_ATTR_S *VoPubAttrS)
    {
        HI_ASSERT(pstVoConfig);
        /*
         * 定义视频输出公共属性结构体
         */
        VoPubAttrS->enIntfType = VO_INTF_MIPI;
        VoPubAttrS->enIntfSync = VO_OUTPUT_USER;
        VoPubAttrS->stSyncInfo.bSynm = 0;
        VoPubAttrS->stSyncInfo.bIop = 1;
        VoPubAttrS->stSyncInfo.u8Intfb = 0;

        VoPubAttrS->stSyncInfo.u16Hmid = 1;
        VoPubAttrS->stSyncInfo.u16Bvact = 1;
        VoPubAttrS->stSyncInfo.u16Bvbb = 1;
        VoPubAttrS->stSyncInfo.u16Bvfb = 1;

        VoPubAttrS->stSyncInfo.bIdv = 0;
        VoPubAttrS->stSyncInfo.bIhs = 0;
        VoPubAttrS->stSyncInfo.bIvs = 0;

        VoPubAttrS->stSyncInfo.u16Hact = 480; // 480: Horizontal effective area. Unit: pixel
        VoPubAttrS->stSyncInfo.u16Hbb = 60;   // 60: Horizontal blanking of the rear shoulder. Unit: pixel
        VoPubAttrS->stSyncInfo.u16Hfb = 50;   // 50: Horizontal blanking of the front shoulder. Unit: pixel
        VoPubAttrS->stSyncInfo.u16Hpw = 10;   // 10: The width of the horizontal sync signal. Unit: pixel
        VoPubAttrS->stSyncInfo.u16Vact = 800; // 800: Vertical effective area. Unit: line
        VoPubAttrS->stSyncInfo.u16Vbb = 24;   // 24: Vertical blanking of the rear shoulder.  Unit: line
        VoPubAttrS->stSyncInfo.u16Vfb = 20;   // 20: Vertical blanking of the front shoulder.  Unit: line
        VoPubAttrS->stSyncInfo.u16Vpw = 4;    // 4: The width of the vertical sync signal. Unit: line
        VoPubAttrS->u32BgColor = pstVoConfig->u32BgColor;
    }
     ...
     * 启动VO到MIPI lcd通路
     */
    HI_S32 SampleCommVoStartMipi(SAMPLE_VO_CONFIG_S *pstVoConfig, VO_PUB_ATTR_S *VoPubAttr)
    {
        HI_S32 s32Ret;

        HI_ASSERT(pstVoConfig);
        StVoPubAttrCfg(pstVoConfig, VoPubAttr);
        s32Ret = SampleCommVoStartDevMipi(pstVoConfig->VoDev, VoPubAttr);
        SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "StartDevMipi FAIL, ret=%x\n", s32Ret);
        /*
         * 获得MIPI设备的宽和高
         */
        s32Ret = SampleCommVoGetWhMipi(VoPubAttr->enIntfSync,
     ...
     * 初始化vi配置
     */
    void ViCfgInit(SAMPLE_VI_CONFIG_S *self)
    {
        HI_ASSERT(self);
     ...
     * 设置VI设备信息
     */
    void ViCfgSetDev(SAMPLE_VI_CONFIG_S *self, int devId, WDR_MODE_E wdrMode)
    {
        HI_ASSERT(self);
     ...
     * 设置VI的PIPE信息
     */
    void ViCfgSetPipe(SAMPLE_VI_CONFIG_S *self, int pipe0Id, int pipe1Id, int pipe2Id, int pipe3Id)
    {
        HI_ASSERT(self);
     ...
     * 设置VI通道
     */
    void ViCfgSetChn(SAMPLE_VI_CONFIG_S *self, int chnId, PIXEL_FORMAT_E pixFormat,
